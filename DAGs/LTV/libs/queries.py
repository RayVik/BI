...
def scoring_segment(table_name, reserach_period=90,
                    currdate=pd.to_datetime(datetime.datetime.now()).date().strftime('%Y-%m-%d'),
                    seed=42):
    ...
    query_base = f'''
        ...
        select 
            ...
            ROW_NUMBER() OVER (
                PARTITION BY CUSTOMER_ID, TRADE_DT
                order by identification_count DESC,
                         cityHash64(toString(IDENTIFICATION_INDEX), {seed})
            ) as rn_identification
        ...
        select 
            ...
            identification_count.IDENTIFICATION_INDEX as IDENTIFICATION_INDEX,
            region_count.REGION_NAME_EN as REGION_NAME_EN,
            casstickid_count.CASSTICKID as CASSTICKID
